...
class Evaluator:
    def __init__(
        self, cfg, wandb_run, rollout_callback=None, create_task_env_func=None
    ):
        cfg = deepcopy(cfg)
        self.cfg = cfg
        ...
        try:
            ...
            pass
        p = mp.Process(
            target=rollout_callback,
            args=(cfg, wandb_run, self.child, create_task_env_func),
            daemon=True,
        )
        p.start()
    ...
    def eval_policy(self, policy, value, step=0, normalizer=None):
        self.parent.send(
            [cloudpickle.dumps(policy), cloudpickle.dumps(value), step, normalizer]
        )
...
def default_rollout(cfg, wandb_run, child, create_task_env_func=None):
    ...
    if info_track_keys is not None:
        info_track_keys = (
            [info_track_keys] if isinstance(info_track_keys, str) else info_track_keys
        )
        info_trackers = {key: Tracker(tracker_capacity) for key in info_track_keys}
        info_track_step = {
            key: cfg.info_track_step[idx] for idx, key in enumerate(info_track_keys)
        }
        traj_info_values = {
            key: torch.zeros(cfg.num_envs, dtype=torch.float32, device="cpu")
            for key in info_track_keys
        }
    ...
    with torch.inference_mode():
        while True:
            ...
            current_returns = torch.zeros(
                num_envs, dtype=torch.float32, device=cfg.device
            )
            current_lengths = torch.zeros(
                num_envs, dtype=torch.float32, device=cfg.device
            )
            obs = env.reset()
                ...
                if cfg.info_track_keys is not None:
                    ...
                    for key in cfg.info_track_keys:
                        ...
                        if info_track_step[key] == "last":
                            ...
                            info_trackers[key].update(
                                traj_info_values[key][env_done_indices]
                            )
