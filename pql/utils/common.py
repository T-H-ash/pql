...
def preprocess_cfg(cfg):
    with open_dict(cfg):
        # breakpoint()
        if cvd := os.environ.get("CUDA_VISIBLE_DEVICES"):
            cvd_list = list(map(int, cvd.split(",")))
            cfg.available_gpus = cvd_list
        else:
            cfg.available_gpus = range(torch.cuda.device_count())  # all gpus
...
def check_device(cfg):
    ...
    if len(device_set) > len(cfg.available_gpus):
        assert "Invalid CUDA device: id out of range"
    for gpu_id in device_set:
        if gpu_id >= len(cfg.available_gpus):
