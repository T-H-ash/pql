...
def create_additional_buffer(capacity, device="cuda"):
    if isinstance(capacity, int):
        capacity = (capacity,)
    buf_mean_q = torch.empty((*capacity, 1), dtype=torch.float32, device=device)
    buf_target_q = torch.empty((*capacity, 1), dtype=torch.float32, device=device)
    return buf_mean_q, buf_target_q
...
class ReplayBuffer:
    def __init__(self, capacity: int, obs_dim: int, action_dim: int, device="cpu"):
        ...
        # additional metrics
        self.buf_meanq, self.buf_targetq = create_additional_buffer(capacity=self.capacity, device=device)

    @torch.no_grad()
    def add_to_buffer(self, trajectory, trajectory_additional=None):
        ...
        if trajectory_additional is not None:
            meanq, targetq = trajectory_additional
            meanq = meanq.reshape(-1, 1)
            targetq = targetq.reshape(-1, 1)
        ...
        if p > self.capacity:
            ...
            if trajectory_additional is not None:
                self.buf_meanq[self.next_p : self.capacity] = meanq[: self.capacity - self.next_p]
                self.buf_targetq[self.next_p : self.capacity] = targetq[: self.capacity - self.next_p]
            ...
            if trajectory_additional is not None:
                self.buf_meanq[0:p] = meanq[-p:]
                self.buf_targetq[0:p] = targetq[-p:]
            ...
            if trajectory_additional is not None:
                self.buf_meanq[self.next_p : p] = meanq
                self.buf_targetq[self.next_p : p] = targetq
