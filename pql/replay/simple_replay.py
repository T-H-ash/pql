...
class ReplayBuffer:
    def __init__(self, capacity: int, obs_dim: int, action_dim: int, storage_sampling_probability, device="cpu"):
        ...
        self.storage = Storage(obs_dim=obs_dim, action_dim=action_dim, prob=storage_sampling_probability)

    @torch.no_grad()
    def add_to_buffer(self, trajectory):
        self.storage.add_to_buffer(trajectory)
...
def create_storage(capacity, obs_dim, action_dim, device="cuda"):
    if isinstance(capacity, int):
        capacity = (capacity,)
    buf_obs_size = (*capacity, obs_dim) if isinstance(obs_dim, int) else (*capacity, *obs_dim)
    buf_obs = torch.empty(buf_obs_size, dtype=torch.float32, device=device)
    buf_action = torch.empty((*capacity, int(action_dim)), dtype=torch.float32, device=device)
    buf_reward = torch.empty((*capacity, 1), dtype=torch.float32, device=device)
    return buf_obs, buf_action, buf_reward


class Storage:
    def __init__(self, obs_dim: int, action_dim: int, capacity: int = int(1e6), device="cpu", prob=4e-3):
        self.obs_dim = obs_dim
        if isinstance(obs_dim, int):
            self.obs_dim = (self.obs_dim,)
        self.action_dim = action_dim
        self.device = device
        self.cur_capacity = 0  # next pointer
        self.capacity = int(capacity)
        self.prob = prob

        ret = create_storage(capacity=self.capacity, obs_dim=obs_dim, action_dim=action_dim, device=device)
        self.buf_obs, self.buf_action, self.buf_reward = ret
    ...
    def add_to_buffer(self, trajectory):
        ...
        obs, actions, rewards, _next_obs, _dones = trajectory
        obs = obs.reshape(-1, *self.obs_dim)
        actions = actions.reshape(-1, self.action_dim)
        rewards = rewards.reshape(-1, 1)
        p = self.cur_capacity + rewards.shape[0]

        if p > self.capacity:
            raise StorageFullError

        self.buf_obs[self.cur_capacity : p] = obs
        self.buf_action[self.cur_capacity : p] = actions
        self.buf_reward[self.cur_capacity : p] = rewards

        self.cur_capacity = p  # update pointer

    def export(self):
        return (
            self.buf_obs[: self.cur_capacity].cpu().numpy(),
            self.buf_action[: self.cur_capacity].cpu().numpy(),
            self.buf_reward[: self.cur_capacity].cpu().numpy(),
        )
